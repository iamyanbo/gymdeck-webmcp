...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const planSchema = objectSchema({ name: { type: "string" }, goal: { type: "string" }, daysPerWeek: { type: "integer", minimum: 2, maximum: 5 } }, ["name", "goal", "daysPerWeek"]);
    const saveGeneratedPlan = (name: string, goal: string, daysPerWeek: number, verb: "created" | "replaced") => {
      const count = Math.max(2, Math.min(5, daysPerWeek));
      ...
      commit(`Agent ${verb} the ${name} training plan.`, "Agent", (draft) => {
        draft.plan = { id: uid("plan"), name, goal, days: Array.from({ length: count }, (_, index) => ({ id: uid("day"), label: `Day ${index + 1}`, focus: index % 2 === 0 ? "Upper body" : "Lower body", exercises: splits[index].map((id) => { const item = draft.library.find((exercise) => exercise.id === id)!; return { id: uid("prescription"), libraryId: item.id, name: item.name, sets: 3, reps: 8, weight: 0, restSeconds: 90 }; }) })) };
      });
      return `${verb === "created" ? "Created" : "Replaced the saved plan with"} ${name}, containing ${count} training days.`;
    };
    const tools: ModelContextTool[] = [
      ...
      { name: "get_training_plan", title: "Get training plan", description: "Read the complete saved training plan, including every day, focus, exercise, prescription, and cardio block. A response with id 'plan-empty' and zero days means the workspace has no saved plan yet.", inputSchema: objectSchema({}), annotations: readOnly, execute: async () => JSON.stringify(stateRef.current.plan) },
      ...
      { name: "create_training_plan", title: "Create first training plan", description: "Create and save a 2–5 day training plan only when GymDeck has no saved plan. This initializes a fresh workspace and never overwrites or removes an existing plan.", inputSchema: planSchema, annotations: write, execute: async ({ name, goal, daysPerWeek }) => {
        if (stateRef.current.plan.id !== "plan-empty" || stateRef.current.plan.days.length > 0) return `GymDeck already has a saved plan named ${stateRef.current.plan.name}. No changes were made. Use replace_training_plan only if the user explicitly asks to replace it.`;
        return saveGeneratedPlan(String(name), String(goal), Number(daysPerWeek), "created");
      } },
      { name: "replace_training_plan", title: "Replace saved training plan", description: "Replace GymDeck’s existing saved plan with a new 2–5 day plan only when the user explicitly asks to replace it. Workout history is preserved.", inputSchema: planSchema, annotations: write, execute: async ({ name, goal, daysPerWeek }) => saveGeneratedPlan(String(name), String(goal), Number(daysPerWeek), "replaced") },
