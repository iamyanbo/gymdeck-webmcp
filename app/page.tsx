    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "plan" && <div className="page-content single-page">
          ...
          <section className="plan-note"><span>✓</span><div><strong>Flexible by design</strong><p>Create, reorder, and adapt each day around your schedule, equipment, and progress.</p></div></section>
        </div>}
      ...
      {activityOpen && <div className="drawer-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setActivityOpen(false); }}><aside className="activity-drawer"><div className="drawer-heading"><div><span className="eyebrow">CHANGE LOG</span><h2>Activity history</h2></div><button onClick={() => setActivityOpen(false)}>×</button></div><p className="drawer-intro">Workout and plan changes are recorded here so you can review or undo them.</p><div className="activity-feed full">{state.activities.map((activity) => <div className="activity-item" key={activity.id}><span className={`activity-icon ${activity.source.toLowerCase()}`}>{activity.source === "Agent" ? "✦" : activity.source === "You" ? "Y" : "•"}</span><div><strong>{activity.source}</strong><p>{activity.message}</p><small>{activity.time}</small></div></div>)}</div><button className="undo-button sticky" onClick={undo} disabled={!canUndo}>↶ Undo last change</button></aside></div>}
    </main>
  );
}
